...
function buildEmailHTML({ companyName, totalSavings, annualSavings, shareUrl, isHighSavings }) {
  const savingsLine =
    totalSavings > 0
      ? `<p style="font-size:32px;font-weight:900;color:#10b981;margin:0 0 4px 0;">
           $${Number(totalSavings).toFixed(0)}
           <span style="font-size:14px;color:#6b7280;">/mo identified</span>
         </p>
         <p style="font-size:13px;color:#6b7280;margin:0 0 24px 0;">
           That's <strong>$${Number(annualSavings).toFixed(0)}</strong> recovered annually.
         </p>`
      : `<p style="font-size:15px;color:#374151;margin:0 0 24px 0;">
           ✅ Your AI stack is already well-optimized. No material savings found right now — we'll alert you when that changes.
         </p>`;

  const cta = shareUrl
    ? `<a href="${shareUrl}"
         style="display:inline-block;background:#032f24;color:#fff;text-decoration:none;
                font-weight:800;font-size:13px;padding:14px 28px;border-radius:12px;margin-top:8px;">
  ...
  const highSavingsNote = isHighSavings
    ? `<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:16px;margin-top:20px;">
         <p style="margin:0;font-size:12px;font-weight:700;color:#065f46;">
           🚀 High-savings case detected. A Credex advisor may reach out to help you implement these changes.
         </p>
       </div>`
    : "";
...
<head>
  ...
  <title>Your AI Spend Audit — Credex</title>
</head>
      ...
      <table width="100%" style="max-width:560px;background:#ffffff;border-radius:24px;overflow:hidden;border:1px solid #e5e7eb;">
        ...
        <tr><td style="padding:32px 36px;">
          ...
          <p style="font-size:11px;color:#9ca3af;margin:0;line-height:1.6;">
            Pricing benchmarks verified May 2026. This audit is informational only.<br><br>
...
export default async function handler(req, res) {
  ...
  const { to, companyName, totalSavings, annualSavings, shareUrl, isHighSavings } = req.body;
